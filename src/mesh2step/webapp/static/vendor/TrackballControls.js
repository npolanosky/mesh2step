/* TrackballControls for three.js r128 (classic global build).
 *
 * Vendored from the official three.js examples (jsm/controls/TrackballControls,
 * r128) and adapted to the non-module global pattern used by the other vendored
 * controls here: it attaches to `THREE.TrackballControls` and extends
 * `THREE.EventDispatcher`, matching this repo's OrbitControls.js.
 *
 * Free-orbit (trackball) rotation with NO up-axis lock — the counterpart to
 * OrbitControls' constrained turntable. Same public surface the viewer needs:
 * `.target` (Vector3), `.object`, `.update()`, `.enabled`, `.dispose()`.
 */
(function () {
  ...
  class TrackballControls extends THREE.EventDispatcher {
    ...
    constructor(object, domElement) {
      ...
      function onMouseDown(event) {
        ...
        switch (mouseAction) {
          case THREE.MOUSE.DOLLY: _state = STATE.ZOOM; break;
          case THREE.MOUSE.ROTATE: _state = STATE.ROTATE; break;
          case THREE.MOUSE.PAN: _state = STATE.PAN; break;
      ...
      function onTouchEnd(event) {
        switch (_pointers.length) {
          case 0: _state = STATE.NONE; break;
          case 1:
            _state = STATE.TOUCH_ROTATE;
            _moveCurr.copy(getMouseOnCircle(event.pageX, event.pageY));
            _movePrev.copy(_moveCurr);
      ...
      function removePointer(event) {
        ...
        for (let i = 0; i < _pointers.length; i++) {
          if (_pointers[i].pointerId == event.pointerId) { _pointers.splice(i, 1); return; }
        }
      }
      function trackPointer(event) {
        let position = _pointerPositions[event.pointerId];
        if (position === undefined) { position = new THREE.Vector2(); _pointerPositions[event.pointerId] = position; }
